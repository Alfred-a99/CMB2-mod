...
class CMB2_Ajax
{
	...
	public function oembed_handler()
	{
		...
		// Verify our nonce.
		if (! (isset($_REQUEST['cmb2_ajax_nonce'], $_REQUEST['oembed_url']) && wp_verify_nonce($_REQUEST['cmb2_ajax_nonce'], 'cmb2_oembed_nonce'))) {
			die();
		}
		...
		$object_type = isset($_REQUEST['object_type'])
			? sanitize_text_field(wp_unslash($_REQUEST['object_type']))
			: 'post';

		if (! in_array($object_type, ['post', 'user', 'comment', 'term'], true)) {
			$object_type = 'post';
		}
		...
		$html = $this->get_oembed(array(
			...
			'field_id'    => sanitize_text_field(wp_unslash($_REQUEST['field_id'])),
		));
